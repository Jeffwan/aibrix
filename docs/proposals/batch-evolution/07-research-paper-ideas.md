...
**别想着"一篇大而全"——拆成一个组合(本轮已据「A/D 为重点、共置降级」调整):**

> 论文编号 Paper A/B/C 与家族字母 A–E **无关**;"核心"列已注明对应家族。

| 推荐 | Paper | 核心 novelty | 判决 | 风险 | 目标 |
|---|---|---|---|---|---|
| **先发** | **C. Measurement**:跨云/neocloud/spot 的 batch 推理**成本实测**(顺带量化家族 A 的吞吐增益) | 学术界**没有** neocloud $/token + spot 可用性的严肃测量 | 白空间确认 | 低 | SoCC/IMC/MLSys(或 workshop 首发) |
| **主攻(旗舰)= 你的 A+D 合成** | **A. 推理原生、成本最优的 batch 控制面**(#1 KV 级跨区迁移 + #2 调度 + #4 cost-SLO,**并融入家族 A 的 prefix/KV 局部性感知放置**) | 跨 hyperscaler+neocloud+spot+异构 的**推理原生统一控制面** + 跨区 KV 级迁移 | 白空间确认,需打透"对 SageServe 的差异 + 交互效应" | 高 | OSDI/SOSP/NSDI |
| **可选(已降级)** | **B. 多模型在线↔离线隔离保障**(#3) | 异构多模型共置的有界在线影响无人做 | NOVEL,但**仅当你走共置(你已表示可能不做)** | 中 | OSDI/SOSP/MLSys |

一句话:**你最感兴趣的 A、D 正好合成旗舰那篇(D=跨云成本最优 + 推理原生迁移;A=prefix/KV 局部性感知的放置与吞吐);C 打底出数据与成本模型;B(隔离)随共置路径降为可选。**
...
> **本轮优先级调整**:旗舰(下文 Paper A)= 你的 **A+D** 合成,为**主攻**;measurement(Paper C)先发;**多模型隔离(Paper B)随共置降级为可选**(见 `02` 的 P5 / `05` 的 D-B·D-E)。下文各篇技术内容仍有效,仅推荐次序变化。
...
## 7. 一页纸建议(本轮已据「A/D 为重点、共置降级」调整)

- **最稳的两步**:先 **C(measurement)** 占住"neocloud/spot batch 成本"空白并建 cost model;同时把 **家族 A(offline 感知)的吞吐增益**也测进去(prefix 共享/混排在真实负载上的收益)。
- **主攻旗舰 = 你的 A + D 合成一篇**:**推理原生、成本最优的 batch 控制面**——D 给"跨区/跨 provider spot 成本最优 + 秒级 KV 级迁移(#1)",A 给"prefix/KV 局部性感知的放置 + 整批吞吐"。立论靠 **#1 迁移机制 + #2 交互效应 + 直接打 SageServe**。
- **A 单独成篇不建议**:BatchLLM/BlendServe 已占 offline prefix 共享/混排;A 的 research 价值在于**作为旗舰的"局部性感知放置"组件(A×D)**,或并入 C 的测量。
- **B(多模型隔离)= 可选**:NOVEL,但随共置路径降级——**只有你决定走共置(你已表示可能不做)才写**。
- **共同前提**:可信度依赖 `03` 的诚实口径与强基线;**先把评测/测量脚手架搭好**(与你"先把事情了清楚、暂不写功能代码"的节奏一致)。
