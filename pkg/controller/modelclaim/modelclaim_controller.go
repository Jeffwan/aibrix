...
// a GPU through kvcached and vLLM sleep/kvctl runtime actuators.
package modelclaim
...
type ModelClaimReconciler struct {
	...
	// Runtime drives the per-runtime sidecar (activate/deactivate/budget). Injected
	// so the reconcile loop is testable with an in-process fake.
	Runtime RuntimeClient
	// Locality scores how cheap it is to load a model's weights on a node (the
	// Layer 2 node weight-cache signal). Defaults to uniformLocality (no
	// preference) until the node store reports per-node locality.
...
func Add(mgr manager.Manager, _ config.RuntimeConfig) error {
	r := &ModelClaimReconciler{
		...
		Locality: storeLocality{reader: mgr.GetClient()},
	}
	...
	err := ctrl.NewControllerManagedBy(mgr).
		...
		// attach as soon as capacity appears.
		Watches(&corev1.Pod{},
...
// Reconcile drives a ModelClaim towards its desired state. For the Layer 1
// milestone it manages the finalizer, discovers candidate warm pods, and keeps
// status/phase current. Placement (bin-packing onto a warm pod), activation via
// the runtime sidecar, and gateway route registration are layered on next.
...
// listCandidateWarmPods returns running pods that match the model's PoolSelector
...
func (r *ModelClaimReconciler) listCandidateWarmPods(ctx context.Context, pm *modelv1alpha1.ModelClaim) ([]corev1.Pod, error) {
	selector, err := metav1.LabelSelectorAsSelector(pm.Spec.PoolSelector)
	if err != nil {
		return nil, err
	}
	...
	for i := range podList.Items {
		pod := podList.Items[i]
		if pod.Labels[constants.ModelPoolLabelEnabled] != "true" {
			continue
		}
...
func (r *ModelClaimReconciler) recomputeReadiness(pm *modelv1alpha1.ModelClaim) {
	...
	case len(pm.Status.Instances) > 0:
		...
		// model is not yet routable; the gateway 503s/holds until ready.
...
// each. Lack of capacity is not an error (the model stays Pending and
...
func (r *ModelClaimReconciler) ensureActivated(ctx context.Context, pm *modelv1alpha1.ModelClaim, candidates []corev1.Pod) error {
	...
	for desiredReplicas(pm) > int32(len(pm.Status.Instances)) {
		...
		if err != nil {
			// No spare warm pod right now; remain Pending and retry on requeue.
			return nil
		}

		// Carry the CRD's KV-sharing semantics to the runtime so the node-level
		// reclaim loop can rebuild this model's demand without reading CRDs.
		semantics := ToModelDemand(pm, 0)
		resp, aerr := r.Runtime.Activate(ctx, pod.Status.PodIP, DefaultRuntimePort, &ActivateRequest{
			...
			KVMinBytes:       kvMin,
			KVMaxBytes:       kvMax,
			KVPolicy:         string(semantics.Policy),
			KVShares:         semantics.Shares,
			Priority:         semantics.Priority,
			AdditionalConfig: pm.Spec.AdditionalConfig,
		})
		if aerr != nil {
...
// routable, so the gateway can hold or return retryable backpressure instead of
// routing to a not-ready engine.
...
func modelPoolPodFilter() predicate.Predicate {
	isModelPoolPod := func(labels map[string]string) bool {
		...
		return labels[constants.ModelPoolLabelEnabled] == "true"
	}
...
// when warm-pool capacity changes. A pod add may let a pending model attach; a
