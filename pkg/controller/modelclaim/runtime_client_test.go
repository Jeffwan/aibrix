...
func TestHTTPRuntimeActivate(t *testing.T) {
	...
	resp, err := c.Activate(context.Background(), host, port, &ActivateRequest{ModelName: "m1", IPCName: "kvc_m1"})
...
func TestHTTPRuntimeDeactivate(t *testing.T) {
	...
	c, host, port := clientForServer(srv)
	err := c.Deactivate(context.Background(), host, port, &DeactivateRequest{ModelName: "m1", Mode: DeactivateWarm})
	require.NoError(t, err)
	assert.Equal(t, "warm", gotMode)
}

func TestHTTPRuntimeSetKVBudget(t *testing.T) {
	var got KVBudgetRequest
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "/v1/runtime/models/kv_budget", r.URL.Path)
	...
	c, host, port := clientForServer(srv)
	err := c.SetKVBudget(context.Background(), host, port, &KVBudgetRequest{ModelName: "m1", KVMaxBytes: 1 << 30})
	require.NoError(t, err)
	assert.Equal(t, int64(1<<30), got.KVMaxBytes)
}
...
func TestHTTPRuntimeListModels(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(map[string][]ModelInfo{
			"models": {{ModelName: "m1", Port: 9001, Phase: "active"}},
		})
	}))
	defer srv.Close()
