...
func (c *Store) addPod(obj interface{}) {
	...
	for servedModel, port := range modelClaims {
		c.modelClaimState.set(podKey, servedModel, port)
		if port > 0 {
...
func (c *Store) updatePod(oldObj interface{}, newObj interface{}) {
	...
	if oldOk || existed {
		...
		c.modelClaimState.clearPod(utils.GeneratePodKey(oldPod.Namespace, oldPod.Name))
	}
	...
	if (newOk || len(newModelClaims) > 0) && !newIsWorker {
		...
		for servedModel, port := range newModelClaims {
			c.modelClaimState.set(newPodKey, servedModel, port)
			if port > 0 {
				c.addPodAndModelMappingLocked(metaPod, servedModel)
			}
		}
...
func (c *Store) deletePod(obj interface{}) {
	...
	c.modelClaimState.clearPod(utils.GeneratePodKey(namespace, name))
