...
func TestAddPodRegistersModelClaim(t *testing.T) {
	...
	assert.True(t, c.IsModelClaim("qwen-0.5b"))
	assert.False(t, c.IsModelClaimNotRoutable("qwen-0.5b"))
}
...
func TestUpdatePodAddsAndRemovesModelClaim(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, 1, b.Len())
	assert.False(t, c.IsModelClaim("m-a"))
	assert.True(t, c.IsModelClaim("m-b"))
}

func TestModelClaimKnownButNotRoutable(t *testing.T) {
	c := NewForTest()
	c.addPod(warmModelClaimPod("p1", "default", map[string]int{"m-live": 9001, "m-sleeping": 0}))

	assert.True(t, c.IsModelClaim("m-live"))
	assert.True(t, c.IsModelClaim("m-sleeping"))
	assert.False(t, c.IsModelClaimNotRoutable("m-live"))
	assert.True(t, c.IsModelClaimNotRoutable("m-sleeping"))

	assert.True(t, c.HasModel("m-live"))
	assert.False(t, c.HasModel("m-sleeping"), "non-routable ModelClaim must have no routable pods")
...
func TestModelClaimStateClearedOnPodDelete(t *testing.T) {
	...
	c.addPod(pod)
	require.True(t, c.IsModelClaimNotRoutable("m"))

	c.deletePod(pod)
	assert.False(t, c.IsModelClaimNotRoutable("m"))
	assert.False(t, c.IsModelClaim("m"))
}
