...
// buildRuntimeSidecarContainer creates the runtime sidecar container with the given configuration.
// This is shared logic used by all webhooks (Pod, Deployment, StormService).
func buildRuntimeSidecarContainer(image, engineType string) corev1.Container {
...
// Shared across all webhooks for consistent engine detection.
...
// containsContainer checks if a container with the given name exists in the container list.
// Used to avoid duplicate sidecar injection.
