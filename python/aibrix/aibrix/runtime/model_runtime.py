...
controller drives this runtime over HTTP to:
  ...
  * deactivate it (warm = release KV via kvctl, evict = vLLM sleep, stop = kill),
  * set its kvcached KV budget (the resource-manager actuator), and
  * list the models currently resident on the pod.

The engine launcher and KV controller are pluggable so the agent runs and is
fully testable without a GPU (MockEngineLauncher + MockKVController).
"""
...
class ModelInstance:
    ...
    ipc_name: str
    engine: str = "vllm"
    phase: str = "active"  # active | warm | sleeping
    pid: Optional[int] = None
    kv_min_bytes: int = 0
    kv_max_bytes: int = 0
    # KV-sharing semantics from the CRD (Spec.KV / Spec.Priority), carried in the
    # activate request so the node reclaim loop can rebuild ModelDemand locally.
    kv_policy: str = kv_reclaim.POLICY_GUARANTEED
    kv_shares: int = 1
    priority: int = 0
    # Underlying process handle (None for mock / sleeping); excluded from repr.
    proc: Optional[object] = field(default=None, repr=False)


class ModelNotFound(Exception):
    """Raised when an operation targets a model the agent does not host."""
...
class EngineLauncher(ABC):
    """Launches/stops/sleeps an engine process for a model."""
    ...
    def sleep(self, inst: ModelInstance, level: int) -> None:
        """Demote weights via vLLM sleep (level 1 -> CPU DRAM, 2 -> discard)."""

    @abstractmethod
    def wake(self, inst: ModelInstance) -> None:
        """Restore a slept model."""


class KVController(ABC):
    """Sets a model's kvcached KV budget (written through kvctl into /dev/shm)."""

    @abstractmethod
    def set_limit(
        self, ipc_name: str, kv_max_bytes: int, kv_min_bytes: int = 0
    ) -> None: ...
...
class SubprocessEngineLauncher(EngineLauncher):
    ...
    def launch(self, inst, artifact_url, additional_config):
        import subprocess
    ...
    def sleep(self, inst, level):
        _engine_post(inst.port, f"/sleep?level={level}")

    def wake(self, inst):
        _engine_post(inst.port, "/wake_up")


class KvctlController(KVController):
    """Drives kvcached's kvctl CLI to set a per-model KV budget."""

    def set_limit(self, ipc_name, kv_max_bytes, kv_min_bytes=0):
        import subprocess

        subprocess.run(["kvctl", "limit", ipc_name, str(kv_max_bytes)], check=False)


def _engine_post(port: int, path: str) -> None:
    try:
        import httpx

        httpx.post(f"http://127.0.0.1:{port}{path}", timeout=30)
    except Exception as exc:  # best-effort; log and continue
        logger.warning("engine call %s on port %d failed: %s", path, port, exc)
...
class MockEngineLauncher(EngineLauncher):
    ...
    def wake(self, inst):
        self.woke.append(inst.model_name)
...
class MockKVController(KVController):
    ...
    def set_limit(self, ipc_name, kv_max_bytes, kv_min_bytes=0):
        self.limits[ipc_name] = (kv_max_bytes, kv_min_bytes)
...
class ModelRuntime:
        ...
        *,
        ...
        with self._lock:
            ...
            inst = ModelInstance(
                ...
                engine=engine,
                kv_min_bytes=kv_min_bytes,
                kv_max_bytes=kv_max_bytes,
                kv_policy=kv_policy or kv_reclaim.POLICY_GUARANTEED,
                kv_shares=kv_shares if kv_shares > 0 else 1,
                priority=priority,
            )
            inst.pid = self._launcher.launch(inst, artifact_url, additional_config)
            if kv_max_bytes:
                self._kv.set_limit(inst.ipc_name, kv_max_bytes, kv_min_bytes)
            inst.phase = "active"
    ...
    def deactivate(
        self, model_name: str, mode: str = "stop", sleep_level: int = 1
    ) -> None:
        with self._lock:
            ...
            if inst is None:
                return
            if mode == "warm":
                # Release KV: clamp the budget to the floor; weights stay resident.
                self._kv.set_limit(inst.ipc_name, inst.kv_min_bytes, inst.kv_min_bytes)
                inst.phase = "warm"
            elif mode == "evict":
                self._launcher.sleep(inst, sleep_level)
                inst.phase = "sleeping"
            else:  # stop
    ...
    def set_kv_budget(
        self, model_name: str, kv_max_bytes: int, kv_min_bytes: int = 0
    ) -> None:
        with self._lock:
            inst = self._models.get(model_name)
            if inst is None:
            ...
            self._kv.set_limit(inst.ipc_name, kv_max_bytes, kv_min_bytes)
            inst.kv_max_bytes = kv_max_bytes
            inst.kv_min_bytes = kv_min_bytes
    ...
    def list_models(self) -> List[ModelInstance]:
        ...
        with self._lock:
            ...
            for name in dead:
                ...
                logger.warning(
                    "reaping dead engine for %s (port %d)", name, inst.port
                )
    ...
    def reclaim(
        ...
        """One fast-reclaim-loop tick: compute per-model KV budgets (and weight
        evictions) for this GPU and apply them via kvctl / sleep. demands carries
        each resident model's policy/min/max/shares/priority + current usage
        (the node agent builds it from the CRD + kvtop)."""
        plan = kv_reclaim.compute(capacity_bytes, headroom_bytes, demands)
        self.apply_reclaim(plan)
        return plan

    def apply_reclaim(self, plan: "kv_reclaim.Plan") -> None:
        """Apply a reclaim plan: set each model's KV budget (kvctl) and evict the
        weights of models whose floors did not fit (vLLM sleep)."""
        with self._lock:
            for name, budget in plan.budgets.items():
                inst = self._models.get(name)
                if inst is None:
                    continue
                self._kv.set_limit(inst.ipc_name, budget, inst.kv_min_bytes)
                inst.kv_max_bytes = budget
            for name in plan.evict:
                inst = self._models.get(name)
                if inst is None:
                    continue
                self._launcher.sleep(inst, 1)
                inst.phase = "sleeping"
...
def get_model_runtime() -> ModelRuntime:
    ...
    if _AGENT is None:
        if os.environ.get("AIBRIX_MODEL_RUNTIME_MOCK") == "1":
            _AGENT = ModelRuntime(MockEngineLauncher(), MockKVController())
        else:
            _AGENT = ModelRuntime(SubprocessEngineLauncher(), KvctlController())
    return _AGENT
