...
def make_agent():
    return ModelRuntime(MockEngineLauncher(), MockKVController())


def test_activate_assigns_ipc_port_and_budget():
    agent = make_agent()
    inst = agent.activate(model_name="m1", artifact_url="hf://x", kv_max_bytes=1 << 30)
    assert inst.ipc_name == "kvc_m1"
    assert 20000 <= inst.port < 21000
    assert inst.phase == "active"
    assert inst.pid is not None
    # kv budget written through the (mock) kv controller
    assert agent._kv.limits["kvc_m1"] == (1 << 30, 0)
...
def test_deactivate_warm_releases_kv_keeps_model():
    agent = make_agent()
    agent.activate(
        model_name="m1",
        artifact_url="hf://x",
    ...
    inst = agent.list_models()[0]
    assert inst.phase == "warm"
    # budget clamped to the floor; the model stays resident
    assert agent._kv.limits["kvc_m1"] == (1 << 28, 1 << 28)
    assert agent._launcher.stopped == []


def test_deactivate_evict_sleeps_weights():
    agent = make_agent()
    agent.activate(model_name="m1", artifact_url="hf://x")
    agent.deactivate("m1", mode="evict", sleep_level=2)
    inst = agent.list_models()[0]
    assert inst.phase == "sleeping"
    assert agent._launcher.slept == [("m1", 2)]
...
def test_set_kv_budget():
    agent = make_agent()
    agent.activate(model_name="m1", artifact_url="hf://x")
    agent.set_kv_budget("m1", kv_max_bytes=2 << 30, kv_min_bytes=1 << 29)
    assert agent._kv.limits["kvc_m1"] == (2 << 30, 1 << 29)


def test_set_kv_budget_missing_model_raises():
    agent = make_agent()
    with pytest.raises(ModelNotFound):
        agent.set_kv_budget("nope", kv_max_bytes=1)


def test_reclaim_tick_applies_budgets_and_evicts():
    from aibrix.runtime.kv_reclaim import POLICY_GUARANTEED, ModelDemand

    agent = make_agent()
    for name in ("a", "b", "c"):
        agent.activate(
            model_name=name, artifact_url="hf://x", kv_min_bytes=50, kv_max_bytes=100
        )

    # Floors 150 > capacity 100: lowest-priority c is evicted (sleep), a/b get budgets.
    plan = agent.reclaim(
        100,
        0,
        [
    ...
    assert plan.evict == ["c"]
    # budgets applied through the (mock) kv controller for the survivors
    assert agent._kv.limits["kvc_a"][0] == 50
    assert agent._kv.limits["kvc_b"][0] == 50
    # c's weights demoted via sleep and its phase updated
    assert ("c", 1) in agent._launcher.slept
    c = next(m for m in agent.list_models() if m.model_name == "c")
    assert c.phase == "sleeping"
...
# --------------------------------------------------------------------------- #
# Layer-2 block B/C: cache markers, KV semantics, /dev/shm accounting
# --------------------------------------------------------------------------- #
def test_activate_stores_kv_semantics_and_writes_cache_marker(tmp_path, monkeypatch):
    import json
    ...
    inst = agent.activate(
        model_name="m1",
        artifact_url="huggingface://Org/M1",
        kv_policy="Shared",
        kv_shares=3,
        priority=42,
    )
    assert (inst.kv_policy, inst.kv_shares, inst.priority) == ("Shared", 3, 42)
